    ...
    return rememberLauncherForActivityResult(
        ...
        onResult = { uris ->
            if (currentImageCount >= 5) {
                Toast.makeText(context, "You can't add more than 5 images.", Toast.LENGTH_SHORT).show()
                return@rememberLauncherForActivityResult
            }
            val availableSlots = 5 - currentImageCount
            if (uris.size > availableSlots) {
                Toast.makeText(context, "You can only add $availableSlots more images. The first $availableSlots images will be added.", Toast.LENGTH_LONG).show()
            }
